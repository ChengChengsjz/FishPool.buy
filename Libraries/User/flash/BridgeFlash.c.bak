...
void Threshold_Init(void)
{
	...
	if(strConfig_info.is_config != FLASH_FLAG_READY) {
		// 设置默认阈值
		Threshold.WtrTempThreshold = 25;//设置水温阈值
		Threshold.WtrLevelThreshold = 20;//设置水位阈值
		Threshold.LightThreshold = 100;//设置光照强度阈值
		Threshold.TurbThreshold = 600;//设置浊度阈值
		Threshold.PHMax = 10;//设置PHMAX阈值
		Threshold.PHMin = 5;//设置PHMin阈值
		
		Threshold.FeedHour = 1;//设置投喂时
        Threshold.FeedMinute = 1;//设置投喂分
        Threshold.FeedTime = 10;//设置投喂执行时间
        
        Threshold.OxHour = 1;//设置增氧时
        Threshold.OxMinute = 1;//设置增氧分
        Threshold.OxTime = 10;//设置增氧执行时间
		// 标记已配置并保存
